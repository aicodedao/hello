# AiCodeDao • Next-Gen Agentic Hello ✨ (v5.0)

Ứng dụng Web thế hệ mới theo phong cách **Cyber-Glassmorphism 3.0**, tích hợp **Gemini AI Assistant Chatbot** (hỗ trợ REST API key tuỳ chỉnh & Agentic Mock Streaming), **Holographic Cyber HUD 3D Core Reactor (Canvas 3D Engine)**, **AI Agentic Core HUD Grid**, **Multi-mode Particle & Canvas Engine (4 Chế độ)**, **Web Audio API Synth Engine Pro+ (6 Presets & AI Token Blips)** với Waveform Visualizer, **Hỗ trợ 8 Ngôn ngữ Quốc tế**, **Mini AI Terminal CLI v5.0**, **6 Bảng màu Neon**, **AI Wisdom Quotes Engine**, và triển khai Container **Nginx Alpine (Port 8080)** với **Cloudflare Tunnel Ingress** cho subdomain `hello.aicodedao.xyz`.

---

## 🚀 Tính năng nổi bật thế hệ v5.0

- 🤖 **Gemini AI Assistant Chatbot:**
  - Tương tác thông minh đa lĩnh vực (Agentic Architecture, Cloudflare Tunnel, Docker DevOps, Web Audio, Glassmorphism UI).
  - Tích hợp **Google Gemini REST API** (`gemini-2.5-flash`, `gemini-2.0-flash`, `gemini-1.5-pro`, `gemini-1.5-flash`) với modal cấu hình API Key trực tiếp trên UI (lưu mã hoá `localStorage`).
  - Chế độ **Intelligent Agentic Mock Streaming Fallback** tự động kích hoạt khi chưa nhập API key hoặc mất kết nối mạng.
  - Hiệu ứng typewriter stream mượt mà, định dạng Markdown (tiêu đề, in đậm, danh sách, mã nguồn) kèm nút "Sao chép mã" tiện lợi.
- 🔮 **Holographic Cyber HUD 3D Core Reactor:**
  - Lõi đa diện 3D Icosahedron Wireframe xoay không gian 3 chiều tương tác chuột/cảm ứng theo thời gian thực trên Canvas chuyên dụng.
  - Vòng quỹ đạo ánh sáng đồng tâm `.ring-outer` và `.ring-inner` xoay đa chiều tạo chiều sâu Holographic Sci-Fi.
- 🎵 **Web Audio API Synth Engine Pro+ (6 Presets & AI Sound Effects):**
  - Polyphonic Synth (`Cyber Synth`, `8-Bit Arcade`, `Zen Chime`, `ASMR Click`, `Sci-Fi Holo`, `Muted`) sinh âm thanh toán học động.
  - Hiệu ứng âm thanh tương tác AI: `ai-token` (âm gõ ký tự typewriter tần số cao), `ai-complete` (hợp âm 3 nốt hoàn tất), `ai-error` (báo động trầm).
...
- 💻 **Mini AI Terminal CLI v5.0:** Giao diện dòng lệnh phong cách hacker với thanh Quick Command Chips (`/status`, `/ai`, `/hud3d`, `/apikey`, `/quote`, `/theme`, `/canvas`, `/confetti`, `/agents`, `/sound`, `/ping`, `/clear`, `/share`).
...
├── index.html            # DOM Cyber-Glassmorphism 3.0, 3D Core, AI Chatbot, Agent HUD & CLI
├── style.css             # CSS Tokens, 3D Core, Chat styling, 6 Neon Themes & Responsive
├── script.js             # 3D Icosahedron Engine, Gemini Chat Engine, Web Audio Pro+ & CLI v5.0
